...
// 和从机握手
ConnectionPack shakeHandWithSlave(ServerConfig &config)
{
    constexpr int buffsize = 128;
    int len=sizeof(ConnectionPack);
    config.conn.status = REPL_STATE_CHECK;
    sendToSlave(config);
    ConnectionPack retpack;
    int readLen=0;
    write(config.slave_socket_fd,reinterpret_cast<void *>(&(config.conn)),len);
    readLen = read(config.slave_socket_fd, reinterpret_cast<void *>(&retpack), sizeof(ConnectionPack));
...
{
    write(config.slave_socket_fd, reinterpret_cast<const void *>(&(config.conn)), sizeof(ConnectionPack));
}

// 和从机同步
void syncWithSlave(DataBase &db)
{
    ConnectionPack slaveConn = shakeHandWithSlave(db.config);
...
// 和主机握手
ConnectionPack shakeHandWithMaster(ServerConfig &config)
{
    constexpr int buffsize = 128;
    int len=sizeof(ConnectionPack);
    ConnectionPack retpack;
...
{
    write(config.master_socket_fd, reinterpret_cast<const void *>(&(config.conn)), sizeof(ConnectionPack));
}
...
{
    constexpr int BUFFSIZE = 128;
    int packlen = sizeof(ConnectionPack);
    ConnectionPack retpack;
    while(1)
    {
